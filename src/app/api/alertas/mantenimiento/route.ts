...
// Devuelve el contenido de la vista `mantenimiento_pendiente` en JSON, para
// que n8n lo consuma vía un HTTP Request node y dispare las alertas por correo.
//
...
export async function GET() {
  ...
  const { data, error } = await supabase.from("mantenimiento_pendiente").select("*");
